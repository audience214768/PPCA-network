...
mod imp {
    ...
    pub async fn run_tproxy_tcp(_bind_addr: SocketAddr) -> Result<()> {
        println!("TPROXY 仅支持 Linux，当前平台已跳过");
        // Block forever so the spawned task doesn't exit.
        std::future::pending::<()>().await;
        Ok(())
    }
}

pub use imp::run_tproxy_tcp;
